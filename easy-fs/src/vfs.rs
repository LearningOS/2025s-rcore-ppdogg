...
impl Inode {
    ...
    /// Remove a hard link
    fn remove_link_entry(&self, name: &str, disk_inode: &mut DiskInode) -> Option<u32> {
        // assert it is a directory
        assert!(disk_inode.is_dir());
        let file_count = (disk_inode.size as usize) / DIRENT_SZ;
        let mut dirent = DirEntry::empty();
    ...
    /// Find number of hard link of given name
    fn link_cnt(&self, inode_id: u32, disk_inode: &DiskInode) -> u32 {
        // assert it is a directory
        assert!(disk_inode.is_dir());
        let file_count = (disk_inode.size as usize) / DIRENT_SZ;
        let mut dirent = DirEntry::empty();
    ...
    /// Create a hard link for old file
    ...
    pub fn unlinkat(&self, name: &str) -> bool {
        ...
        let inode_id = self.modify_disk_inode(|root_inode| {
            self.remove_link_entry(name, root_inode)
        });
        ...
        if self.read_disk_inode(|root_inode: &DiskInode| {
            assert!(root_inode.is_dir());
            self.link_cnt(inode_id.unwrap(), root_inode)
        }) == 0 {
            // let _ = self.modify_disk_inode(|root_inode: &DiskInode| {})
        }
        true
    }
    /// Get hard link of a file
    pub fn get_file_link(&self, inode_id: u32) -> u32 {
        let _fs = self.fs.lock();
        self.read_disk_inode(|root_inode: &DiskInode| {
            assert!(root_inode.is_dir());
            self.link_cnt(inode_id, root_inode)
        })
    }
